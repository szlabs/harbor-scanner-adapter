...
// Capability consists of the set of recognized artifact MIME spec and the set of scanner report MIME spec. For example, a scanner capable of analyzing Docker images and producing a vulnerabilities report recognizable by Harbor web console might be represented with the following capability: - consumes MIME spec:   - `application/vnd.oci.image.manifest.v1+json`   - `application/vnd.docker.distribution.manifest.v2+json` - produces MIME spec:   - `application/vnd.scanner.adapter.vuln.report.harbor+json; version=1.0`
type ScannerCapability struct {
	// The set of MIME spec of the artifacts supported by the scanner to produce the reports specified in the \"produces_mime_types\". A given mime type should only be present in one capability item.
	ConsumesMimeTypes []string `json:"consumes_mime_types"`
	// The set of MIME spec of reports generated by the scanner for the consumes_mime_types of the same capability record.
